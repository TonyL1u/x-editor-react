...
export default function XEditor(props: XEditorProps) {
    const { code, onChange, imports, className, style, width = 1000, height = 400, fullscreen = false, showConsole = false, showPreview = false, showToolbox = false } = props;
    ...
    return (
        <XEditorProvider code={code} imports={imports}>
            <SplitPanel vertical className={className} style={mergedStyle}>
                ...
                <SplitPanel width={isFullscreen ? '100%' : width} height={height} style={{ flex: isFullscreen ? 1 : '' }}>
                    ...
                    <Editor width="60%" height="100%" borderless onChange={onChange} style={{ flex: 1 }} />
                </SplitPanel>
